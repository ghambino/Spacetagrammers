...
const App = () => {
  ...
  const likesHandler = (id) => {
    const newArr = [];
    classModifier.map((unit, index) => {
      if (index == id) {
        newArr.push({ ...unit, likes: unit.likes + 1 });
      } else {
        newArr.push(unit);
      }
    });
  ...
  return (
    <div className="container:md p-6 bg-gray-200">
      ...
      <div className="flex gap-8 align-center flex-wrap justify-center mt-6">
        ...
        {fetchData.map((photo, index) => (
          <div
            key={photo.id}
            className="flex flex-col w-64 gap-4 bg-white border-2"
          >
            <img
              src={photo.img_src}
              alt="nasa imagery"
            ...
            <div className="py-2 px-4 flex flex-col gap-2">
              ...
              <p className="">
                <span className="font-bold capitalize">camera name:</span>{" "}
                {photo.camera.name}
              </p>
              <p>
                <span className="font-bold capitalize">image date:</span>{" "}
                {photo.earth_date}
              </p>
              <p>
                <span className="font-bold capitalize">satelite rover:</span>{" "}
                {photo.rover.name}
              </p>
              ...
              <div className="p-4 flex justify-between items-center ">
                <FontAwesomeIcon
                  ...
                  onClick={() => likesHandler(index)}
                  style={{
                    color: classModifier[index].likes > 0 ? "red" : "black",
                  }}
                />
                <span style={{marginLeft: "0"}}>{classModifier[index].likes}</span>
                <FontAwesomeIcon
                  icon={faRetweet}
                  className="text-2xl cursor-pointer"
                  onClick={() => setRetweetActive(photo.id)}
                  style={{
                    color: retweetActive === photo.id ? "blue" : "black",
                  }}
                />
                <FontAwesomeIcon
                  icon={faShareSquare}
                  className="text-2xl cursor-pointer"
                  onClick={() => setShareActive(photo.id)}
                  style={{
                    color: shareActive === photo.id ? "purple" : "black",
                  }}
                />
              </div>
            </div>
